export function patchProps(oldProps: any, newProps: any, el: HTMLElement) {
...
function patchDomProp(el: HTMLElement, key: string, prev: any, next: any) {
  switch (key) {
    case 'class':
      // 暂时认为class就是字符串
    ...
    case 'style':
      ...
      if (!next) {
        ...
        for (const styleName in next) {
          ;(el.style as any)[styleName] = next[styleName]
        }
        if (prev) {
